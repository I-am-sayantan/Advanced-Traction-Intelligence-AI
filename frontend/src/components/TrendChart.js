...
export default function TrendChart({ metrics }) {
  if (!metrics?.trends || Object.keys(metrics.trends).length === 0) {
    return (
      <div className="bg-white border border-slate-100 rounded-xl p-6 shadow-[0_2px_4px_rgba(0,0,0,0.02)]" data-testid="trend-chart-empty">
        <h3 className="text-xs uppercase tracking-wider font-semibold text-slate-500 mb-4">Metric Trends</h3>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-4 mt-4">
        {trendKeys.map((key, i) => (
          <div key={key} className="flex items-center gap-2" data-testid={`trend-legend-${key}`}>
