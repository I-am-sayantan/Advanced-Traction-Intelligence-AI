...
import type { Insights, Signal } from "../types";

interface SignalFeedProps {
  insights: Insights | null | undefined;
}

export default function SignalFeed({ insights }: SignalFeedProps) {
  if (!insights) {
    return (
      <div
      ...
      >
        ...
        <p className="text-sm text-slate-400">
          No signals yet. Upload data and generate insights.
        </p>
      </div>
    );
  }
  ...
  insights.strategic_insights?.slice(0, 2).forEach((si) => {
    signals.push({
      type: "insight",
      icon: TrendingUp,
      color: "text-brand",
    ...
    >
      ...
      <div className="space-y-3">
        {signals.slice(0, 5).map((sig, i) => (
          ...
          >
            <div
              className={`w-8 h-8 ${sig.bg} rounded-lg flex items-center justify-center shrink-0`}
            >
