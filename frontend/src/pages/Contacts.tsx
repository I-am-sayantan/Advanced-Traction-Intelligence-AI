...
import type { Contact, TagCount } from "../types";
  ...
  const fetchData = useCallback(async () => {
    try {
      const [contactsData, tagsData] = await Promise.all([
        apiFetch<Contact[]>(
          `/api/contacts${filterTag ? `?tag=${encodeURIComponent(filterTag)}` : ""}`,
        ),
        apiFetch<TagCount[]>("/api/contacts/tags"),
      ]);
    ...
    try {
      const tagsList = form.tags
        ? form.tags
            ...
            .filter(Boolean)
        : [];
      const result = await apiFetch<Contact>("/api/contacts", {
        method: "POST",
    ...
    try {
      const tagsList = form.tags
        ? form.tags
            ...
            .filter(Boolean)
        : [];
      const result = await apiFetch<Contact>(`/api/contacts/${contactId}`, {
        method: "PUT",
        body: JSON.stringify({ ...form, tags: tagsList }),
      });
      setContacts((prev) =>
        prev.map((c) => (c.contact_id === contactId ? result : c)),
      );
      setEditingId(null);
      setForm({ ...emptyForm });
          ...
          >
            <div>
              ...
              <p className="text-slate-500 mt-1">
                Manage your VC and client network
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <label
                className="bg-white text-slate-700 border border-slate-200 px-4 py-2 rounded-md text-sm font-medium hover:bg-slate-50 transition-all shadow-sm cursor-pointer inline-flex items-center gap-2"
          ...
          <AnimatePresence>
            {(showAdd || editingId) && (
              ...
              >
                ...
                >
                  <h3 className="font-heading font-medium text-slate-900 mb-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
                    <input
                      ...
                      placeholder="Name *"
                      className="px-3 py-2 bg-white border border-slate-200 rounded-md text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 placeholder:text-slate-400"
                    ...
                    <input
                      ...
                      placeholder="Role (e.g., Partner, Analyst)"
                      className="px-3 py-2 bg-white border border-slate-200 rounded-md text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 placeholder:text-slate-400"
                    ...
                    <input
                      ...
                      placeholder="Tags (VC, Client, Advisor...)"
                      className="px-3 py-2 bg-white border border-slate-200 rounded-md text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 placeholder:text-slate-400"
          ...
          {loading ? (
            ...
            >
              ...
              <p className="text-sm text-slate-500">
                Add VCs, clients, and advisors to manage your network.
              </p>
            </div>
          ) : (
            <div
              className="bg-white border border-slate-100 rounded-xl overflow-hidden shadow-[0_2px_4px_rgba(0,0,0,0.02)]"
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50">
                    ...
                    <th className="text-left px-4 py-3 text-xs uppercase tracking-wider font-semibold text-slate-500">
                      Name
                    </th>
                    <th className="text-left px-4 py-3 text-xs uppercase tracking-wider font-semibold text-slate-500">
                      Email
                    </th>
                    <th className="text-left px-4 py-3 text-xs uppercase tracking-wider font-semibold text-slate-500 hidden md:table-cell">
                      Company
                    </th>
                    <th className="text-left px-4 py-3 text-xs uppercase tracking-wider font-semibold text-slate-500 hidden lg:table-cell">
                      Tags
                    </th>
                    <th className="text-left px-4 py-3 text-xs uppercase tracking-wider font-semibold text-slate-500 hidden lg:table-cell">
                      Sent
