...
import type { Dataset, Narrative, NarrativeType } from "../types";

const NARRATIVE_TYPES: NarrativeType[] = [
  {
    id: "traction_statement",
    label: "Traction Statement",
    icon: TrendingUp,
    desc: "One-line killer pitch + expansion",
  },
  {
    id: "vc_email",
    label: "VC Update Email",
    icon: Mail,
    desc: "Professional investor update email",
  },
  {
    id: "executive_summary",
    label: "Executive Summary",
    icon: BarChart3,
    desc: "Board-ready summary with metrics",
  },
  {
    id: "monthly_update",
...
export default function NarrativeGenerator() {
  ...
  const [history, setHistory] = useState<Narrative[]>([]);
    ...
    (async () => {
      try {
        const [ds, narr] = await Promise.all([
          apiFetch<Dataset[]>("/api/data/datasets"),
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const result = await apiFetch<Narrative>("/api/narrative/generate", {
        method: "POST",
        body: JSON.stringify({
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <h1
              ...
              data-testid="narrative-title"
            >
              Narrative Generator
            </h1>
            <p className="text-slate-500 mt-1">
              AI-powered funding narratives from your growth data
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            >
              ...
              {/* Type */}
              <div className="bg-white border border-slate-100 rounded-xl p-5 shadow-[0_2px_4px_rgba(0,0,0,0.02)]">
                <label className="text-xs uppercase tracking-wider font-semibold text-slate-500 mb-3 block">
                  Narrative Type
                </label>
                ...
                {narrative ? (
                    ...
                    <div className="flex items-center justify-between p-5 border-b border-slate-100">
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">
                          {
                            NARRATIVE_TYPES.find((t) => t.id === narrative.type)
                              ?.label
                          }
                        </p>
                      </div>
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-500 max-w-sm mx-auto">
                      Select a dataset and narrative type, then generate
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-slate-400 shrink-0 ml-2">
                            {
                              NARRATIVE_TYPES.find((t) => t.id === n.type)
                                ?.label
                            }
                          </span>
                        </div>
