...
import { PenLine, Image, X, Loader2, Sparkles, Calendar, Tag, Trash2, ChevronDown, TrendingUp, AlertCircle, Lightbulb, ArrowRight } from 'lucide-react';
...
export default function Updates() {
  ...
  const handleAnalyze = async () => {
    setAnalyzing(true);
    try {
      const data = await apiFetch('/api/updates/ai-analyze', {
        method: 'POST',
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <div className="max-w-4xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
            <div>
              <h1 className="font-heading text-3xl font-semibold text-slate-900 tracking-tight" data-testid="updates-title">Startup Journal</h1>
              <p className="text-slate-500 mt-1">Daily updates — AI learns your story and surfaces trends</p>
            </div>
          ...
          <AnimatePresence>
            {showAnalysis && analysis && (
              ...
              >
                <div className="bg-gradient-to-br from-white to-slate-50 border border-indigo-100 rounded-xl p-6 shadow-sm" data-testid="ai-analysis-panel">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <Sparkles className="w-5 h-5 text-brand" strokeWidth={1.5} />
                      <h2 className="font-heading font-medium text-slate-900">AI Journal Analysis</h2>
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${momentumColors[analysis.momentum_signal] || momentumColors.neutral}`}>
                        {analysis.momentum_signal} momentum
                      </span>
                    </div>
                    <button onClick={() => setShowAnalysis(false)} className="text-slate-400 hover:text-slate-600" data-testid="close-analysis-btn">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  {analysis.key_themes?.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-xs uppercase tracking-wider font-semibold text-slate-500 mb-2">Key Themes</h4>
                      <div className="flex flex-wrap gap-1.5">
                        {analysis.key_themes.map((t, i) => (
                          <span key={i} className="inline-flex items-center px-2.5 py-1 rounded-full text-xs bg-brand-light text-brand border border-brand/10 font-medium">{t}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  {analysis.recommended_update_for_investors && (
                    <div className="bg-white border border-slate-200 rounded-lg p-4 mb-4">
                      <h4 className="text-xs uppercase tracking-wider font-semibold text-slate-500 mb-2">Recommended Investor Update</h4>
                      <p className="text-sm text-slate-700 leading-relaxed">{analysis.recommended_update_for_investors}</p>
                    </div>
                  )}

                  {analysis.trend_observations?.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-xs uppercase tracking-wider font-semibold text-slate-500 mb-2">Trend Observations</h4>
                      <div className="space-y-2">
                        {analysis.trend_observations.map((t, i) => (
                          <div key={i} className="flex gap-3 text-sm">
                            <TrendingUp className="w-4 h-4 text-brand shrink-0 mt-0.5" strokeWidth={1.5} />
                            <div>
                              <span className="font-medium text-slate-900">{t.observation}</span>
                              <span className="text-slate-500"> — {t.implication}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {analysis.action_items?.length > 0 && (
                    <div>
                      <h4 className="text-xs uppercase tracking-wider font-semibold text-slate-500 mb-2">Action Items</h4>
                      <div className="space-y-1.5">
                        {analysis.action_items.map((a, i) => (
                          <div key={i} className="flex items-center gap-2 text-sm text-slate-600">
                            <ArrowRight className="w-3.5 h-3.5 text-brand" strokeWidth={2} />
                            <span>{a}</span>
                          </div>
                        ))}
                      </div>
          ...
          >
            ...
            <div className="flex items-center justify-between border-t border-slate-100 pt-3">
              <div className="flex items-center gap-3">
                <label className="cursor-pointer text-slate-400 hover:text-slate-600 transition-colors inline-flex items-center gap-1.5 text-xs font-medium" data-testid="add-image-btn">
              ...
              >
                {posting ? <Loader2 className="w-4 h-4 animate-spin" /> : <PenLine className="w-4 h-4" />}
          ...
          {loading ? (
            ...
            <div className="bg-white border border-slate-100 rounded-xl p-12 text-center shadow-[0_2px_4px_rgba(0,0,0,0.02)]" data-testid="empty-updates">
              <PenLine className="w-12 h-12 text-slate-300 mx-auto mb-4" strokeWidth={1.5} />
              <h2 className="font-heading text-xl font-medium text-slate-900 mb-2">Start your journal</h2>
              <p className="text-sm text-slate-500">Write your first update. AI will learn your story over time.</p>
            </div>
          ) : (
                  ...
                  {update.tags?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {update.tags.map((tag, j) => (
                        <span key={j} className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-slate-100 text-slate-600 font-medium">{tag}</span>
                      ))}
                    </div>
                  )}
                </motion.div>
